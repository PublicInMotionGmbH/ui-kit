...
/**
 * Higher Order Component which provides datafiltering for charts
 *
 * @param ChartComponent
 * @param pathToDataItems
 * @returns {WithFliters}
 */
function WithFiltersHOC (ChartComponent, pathToDataItems = []) {
   ...
   *@class
   */
  class WithFliters extends React.Component {
    state = {
    ...
    onFilterClick = (filteredItem, filteredIndex) => {
      const { legendProps: { onClick } } = this.props
    ...
    render () {
      ...
      return (
        <div style={{height: '100%', width: '100%'}} className={wrapperCls}>
          <Legend
            className={legendCls}
          ...
          <div style={chartStyles} className={chartCls}>
            <ChartComponent {...restProps} data={modifiedData} />
          </div>
        </div>
      )
    }
